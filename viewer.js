...
(function (global, factory) {
    ...
    var TEMPLATE = '<div class="viewer-container">' + '<div class="viewer-canvas" data-action="mix"></div>' + '<div class="viewer-footer">' + '<div class="viewer-title"></div>' +
        '<ul class="viewer-toolbar">' +
        '<li role="button" class="viewer-zoom-in" title="放大" data-action="zoom-in"></li>' +
        '<li role="button" class="viewer-zoom-out" title="缩小" data-action="zoom-out"></li>' +
        '<li role="button" class="viewer-one-to-one" data-action="one-to-one"></li>' +
        '<li role="button" class="viewer-reset" data-action="reset"></li>' +
        '<li role="button" class="viewer-prev" title="上一张" data-action="prev"></li>' +
        '<li role="button" class="viewer-play" title="自动播放" data-action="play"></li>' +
        '<li role="button" class="viewer-next" title="下一张" data-action="next"></li>' +
        '<li role="button" class="viewer-rotate-left" title="左旋转" data-action="rotate-left"></li>' +
        '<li role="button" class="viewer-rotate-right" title="右旋转" data-action="rotate-right"></li>' +
        '<li role="button" class="viewer-ding-wei" title="定位到当前图片" data-action="ding-wei"></li>' +
        ...
        '</ul>' + '<div class="viewer-navbar">' + '<ul class="viewer-list"></ul>' + '</div>' + '</div>' + '<div class="viewer-tooltip"></div>' + '<div role="button" class="viewer-button" data-action="mix"></div>' + '<div class="viewer-player"></div>' + '</div>';
...
// Events
    ...
    var render = {
        ...
        initList: function initList() {
            ...
            this.$images.each(function (i, image) {
                ...
                var src = image.src;
                var url = options.url;
                if('viewer-img'!== $(image).attr('viewer-img')){
                    return;
                }
                if (!src) {
                    return;
                ...
                list.push('<li>' + '<img' + (' src="' + src + '"') + ' data-source-id="'+ image.getAttribute('id') +'" data-action="view"' + (' data-index="' + i + '"') + (' data-original-url="' + (url || src) + '"') + (' alt="' + alt + '"') + '>' + '</li>');
            });
    ...
    var handlers = {
        click: function click(e) {
            var $target = $(e.target);
            if($(e.target).hasClass('viewer-title')){
                return true;
            }
        ...
        start: function start(_ref) {
            ...
            if($(_ref.target).hasClass('viewer-title')){
                return true;
            }
            if ($(target).is('img')&&$(target).attr('viewer-img')=='viewer-img') {
                this.target = target;
                this.show();
            }
        },
    ...
    var methods = {
        dingWei:function () {
            if ($.isFunction(this.options.dingWei)) {
                this.hide();
                this.$element.one(EVENT_HIDE, this.options.dingWei(this.$image.data('sourceId')));
            }
        },
        /**
        ...
        view: function view(index) {
            ...
            var $image = $('<img src="' + $img.data('originalUrl') + '"  data-source-id="'+ $img.data('sourceId') +'" alt="' + alt + '">');
            ...
            this.$element.one(EVENT_VIEWED, function () {
                ...
                $title.html(_this3.options.renderTitle(alt));
            });
        ...
        update: function update() {
            ...
            if (this.isImg) {
                ...
                $images = $element.find('img[viewer-img="viewer-img"]');
                this.$images = $images;
    ...
    var Viewer = function () {
        ...
        _createClass(Viewer, [{
            ...
            value: function init() {
                ...
                var $images = isImg ? $element : $element.find('img[viewer-img="viewer-img"]');
